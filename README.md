...
python scripts/12_integrated_resilience_artifacts.py
python scripts/13_compare_hazard_overlays.py
...
For online map tile rendering instead of offline-safe geometry rendering, run:

```bash
python scripts/14_generate_study_context_map.py
```

To refresh cached administrative boundaries:

```bash
python scripts/14_generate_study_context_map.py --refresh-cache
```

## Manuscript v3 Artifact Pipeline

The integrated resilience manuscript layer is generated with:

```bash
python scripts/12_integrated_resilience_artifacts.py
```

This step exports reviewer-facing artifacts under `outputs/tables/`, including:

- `table_v3_structural_breaks.csv`
- `table_v3_weather_stress_summary.csv`
- `table_v3_hazard_screening.csv`
- `table_v3_pca_pressure_loadings.csv`
- `table_v3_pca_pressure_top_months.csv`
- `table_v3_event_window_tests.csv`
- `table_v3_basis_risk_markers.csv`
- `table_v3_empirical_layers.csv`

It also updates manuscript traceability memos:

- `docs/paper_v3_statistical_artifact_audit.md`
- `docs/paper_v3_reviewer_concern_alignment.md`
- `outputs/appendix/v3_missing_artifacts_note.md`

## Figure Pipeline (Study-Context Map)

The manuscript context map is generated by:
...
- cached geodata: `data/raw/geospatial_cache/`

Optional geospatial dependencies (only required for map generation) can be installed with:
...
- integrated resilience reviewer-alignment tables and appendix notes (`table_v3_*` and `v3_missing_artifacts_note.md`)
- study-context map figure and map metadata for manuscript integration
