...
\color{blue}
Cocoa-producing territories are exposed to more than price risk alone. In global value chains, farmers often capture a disproportionately low share of total value added—sometimes less than 7\%—leaving them as highly vulnerable price-takers heavily exposed to international volatility \citep{CocoaBarometer2015, WCF2012}. In resilience-oriented food-systems analysis, harm depends on how these market shocks interact with exposure, sensitivity, and adaptive capacity rather than on any single disturbance taken in isolation \citep{Turner2003,Adger2006,Tendall2015,Zurek2022}. For cocoa smallholders in Santander, Colombia, that interaction is evident in two connected domains: benchmark volatility transmitted through international commodity markets, and localized environmental stress arising from rainfall disruption, transport damage, and other disaster-related conditions. These physical realities—such as hydrometeorological events triggering landslides on unpaved mountainous roads—interfere with harvest and marketing. Crucially, they create localized basis risk, where realized local prices deviate sharply from global benchmarks precisely because of these territorial disruptions.

The empirical literature usually separates these domains. Cocoa and commodity-market studies explain pass-through from world prices, exchange-rate exposure, and uneven adjustment along the pricing chain \citep{TsowouGayi2019,Jumah2001,MeyerCramon2004,JimenezRodriguez2022}. A parallel body of work documents climatic suitability, water and heat stress, and the buffering role of agroforestry and diversified production systems in cocoa landscapes \citep{Schroth2016,Lahive2019,Niether2020}. What remains less developed is an empirical design that traces how benchmark shocks propagate to producer-linked prices while also incorporating reliable information on environmental stress and disaster conditions within the same territorial setting. That gap matters for academic interpretation and for smallholders, cooperatives, local traders, and territorial planners who must manage compounding vulnerability when market instability and localized environmental disruptions overlap.

This article addresses that gap by integrating two earlier manuscript lines into one coherent study. The first line presents a supply-chain transmission analysis that links Colombian cocoa prices, world cocoa benchmarks, downstream European chocolate prices, the COP/USD exchange rate, and Brent oil prices. The second line is a weather-disaster-resilience extension that uses environmental variables and the official Santander event registry. The design is layered on purpose. It first estimates the core market-transmission mechanism in the fullest aligned monthly window. It then adds environmental stress in two complementary ways: a full-window weather-stress index built from local NASA POWER anomalies, and a nested disaster extension for August 2021--July 2024 that treats registry-based hazards as territorial exposure markers rather than as a second benchmark-like transmission system. Because the disaster data are shorter, irregular, partly zero-inflated, and partly synthetic via principal component analysis (PCA), they are analyzed using descriptive overlays, parsimonious robustness checks, and an exploratory event window rather than being forced into a co-equal market specification.

The paper makes three concrete contributions. First, it keeps the benchmark-transmission logic of the original manuscript while grounding the resilience framing in observed hazard dynamics. Second, it shows how disaster information can be incorporated without overstating the evidence: hydrometeorological event counts are the most defensible monthly episode marker in the nested window, while a PCA-based multi-hazard indicator is most useful as a contextual territorial-pressure overlay. Third, it reframes the findings in ecological-economics terms by treating the cocoa chain as a socio-ecological system (SES) in which market transmission defines the baseline exposure, and environmental stress conditions how that exposure is absorbed or amplified.
...
Resilience in commodity systems is not adequately captured by prices, agronomy, or institutions taken separately \citep{Niether2020,Mechler2025}. Vulnerability frameworks instead emphasize that shocks matter through their interaction with sensitivity and adaptive capacity \citep{Turner2003,Adger2006}. In this context, we adopt a polycrisis or multi-hazard perspective to define a Socio-Ecological Systems (SES) vulnerability framework. We treat global market transmission as the primary mechanism of exposure, while territorial and environmental stressors serve as variables that condition the system's sensitivity. This perspective is consistent with food-systems research that treats market, environmental, and institutional pressures as linked, compounding processes across scales \citep{Ericksen2008,Tendall2015,Zurek2022}.
...
From the environmental side, the relevant question is not whether every localized hazard directly sets monthly global cocoa prices. Rather, it is whether climatic and disaster-related stressors define the localized capacity to absorb concurrent market shocks. The literature emphasizes buffer capacity to explain this \citep{IfejikaSperanza2013}. Buffer capacity can operates in two dimensions for cocoa farmers: ecological buffer capacity, where agroforestry systems and biodiversity mitigate the physical impacts of climate extremes compared to monocultures \citep{VaastSomarriba2014, Schroth2016, Niether2020}; and socio-institutional buffer capacity, where farmer networks and cooperatives modify the impact of market shocks by providing credit, shared transport, or collective bargaining when physical logistics (e.g., roads) are washed out \citep{Jacobi2014, Mithofer2017}. Furthermore, households' subjective perception of their capacity to adapt often dictates their actual resilience during overlapping crises \citep{Jones2016}. Read together, this literature suggests that vulnerability in cocoa is jointly shaped by global market dependence and locally uneven buffering capacity.

This methodological distinction is consistent with the disaster-risk literature. Composite resilience and hazard-pressure metrics are often used to summarize territorial stress when no single indicator adequately captures local disruption \citep{Cutter2008, Parsons2021}, but such indices are normally interpreted as contextual constructs whose meaning depends on spatial scope, variable selection, and aggregation choices \citep{Tate2012} rather than as canonical market series \citep{Parsons2021,Garschagen2021}. That logic fits the present article. The disaster layer is intended to identify periods of intensified territorial pressure and to enrich the interpretation of resilience, not to mimic the stochastic properties of the benchmark price system.

The ecological-economics relevance of this paper lies in its joint interpretation, addressing a critical void in which market-oriented transmission studies and climate-oriented adaptation research remain traditionally bifurcated \citep{TsowouGayi2019, Schroth2016, Niether2020}. This contribution is primarily empirical: it operationalizes a bridge between global benchmark transmission and localized territorial stress in a setting where data limitations have previously precluded integrated causal analysis \citep{talero_sarmiento_etal_2025, Morales2022, JimenezRodriguez2022}. Consequently, environmental stress is introduced into this study in a disciplined, layered manner to avoid over-parameterizing the short aligned sample \citep{hakkio_rush_1991, lutkepohl_2005}. Weather anomalies serve as full-window contextual indicators, while official disaster records are used within a nested window in which data overlap is sufficient for robust screening \citep{Parsons2021, Garschagen2021}. Hazard claims are strictly limited to what the territorial data can substantiate—specifically, hydrometeorological and broader multi-hazard disruptions—rather than offering unsupported generalities about all natural hazards \citep{DeAngeli2022, Birkmann2023}.

To fill this analytical gap, the empirical design is guided by three core expectations derived from SES and vulnerability frameworks \citep{Turner2003, Adger2006, Mechler2025}. First, world cocoa returns are expected to transmit positively and robustly to Colombian producer-linked returns, establishing global benchmark volatility as the primary mechanism of exposure \citep{TsowouGayi2019, Poku2017}. Second, the European downstream chocolate index should adjust more slowly and less strongly than the producer-linked series, indicating uneven absorption of benchmark volatility along the pricing chain, with smallholders acting as the primary shock-absorbers \citep{JimenezRodriguez2022, Morales2022}. Third, environmental stress should manifest primarily as a contextual amplifier of exposure—identified through discrete episodes of intensified territorial pressure—rather than as a stable, month-to-month substitute for the core benchmark transmission channel \citep{Garschagen2021, Djalante2011, talero_sarmiento_etal_2025}.
...
Table~\ref{tab:hazard_screening} formalizes the screening of possible contextual hazard overlays in the 35-month nested return window. Earthquake counts are too sparse to support direct integration. Hydrometeorological events are observed in 30 of the 35 months and peak in October 2022, making them the most viable monthly territorial episode marker among the candidate counts.
...
This article integrates two earlier manuscript versions into one publication-oriented analysis of cocoa price transmission and territorial resilience in Santander, Colombia. The main conclusion is clear: international cocoa benchmarks remain the dominant short-run driver of Colombian producer-linked prices, but the vulnerability implications of that transmission are better understood when environmental stress is incorporated into the design. Weather anomalies matter mainly as contextual stress. In the shorter disaster window, hydrometeorological event counts provide the most defensible monthly territorial episode marker, while the PCA-based multi-hazard index remains useful as a robustness overlay and event-screening device. The October 2022 peak shared by both signals is associated with a significant mean shift in domestic cocoa returns, but the disaster layer is better interpreted as contextual territorial exposure than as a co-equal transmission series.

The contribution is therefore empirical integration with methodological restraint, not headline novelty. The paper shows how a benchmark-transmission framework can be linked to resilience analysis without making claims beyond what the data support. The approach is most transferable to territories and commodities where producers are tightly tied to global benchmarks and also exposed to recurrent environmental disruption under short and uneven overlap. Future work should test whether longer local price histories, farm-gate panels, independently identified hazard episodes, or richer contracting data can identify which institutional arrangements actually buffer joint market-and-environmental shocks. For now, the most defensible practical lesson is that producer resilience requires attention to both price transmission and territorial exposure, including coordination mechanisms, diversification, early-warning systems, risk-sharing instruments, and adaptation planning that treats these dimensions as connected.
