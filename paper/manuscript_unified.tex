...
\title{Cocoa Price Volatility, Supply-Chain Transmission, and Territorial Exposure: Evidence from Colombia, International Benchmarks, and European Downstream Prices}
...
\begin{abstract}
This study examines how cocoa price volatility is transmitted across the supply chain and what that transmission implies for smallholder vulnerability in Colombia. The analysis asks whether movements in international cocoa benchmarks are transmitted to Colombian domestic cocoa prices and European downstream chocolate prices, and whether local exposure conditions should be treated as direct price drivers or as contextual factors that shape vulnerability. The dataset harmonizes monthly information from AgroNet weekly cacao reference prices for Colombia, the World Bank cocoa benchmark, the European Union (EU) chocolate Harmonised Index of Consumer Prices (HICP), the COP/USD exchange rate, Brent oil prices, and National Aeronautics and Space Administration (NASA) POWER weather variables for San Vicente de Chucur\'{\i}, Santander. The empirical design combines long-run descriptive context with an aligned monthly estimation window for August 2021 to December 2025, using transparent block-specific imputation for isolated missing observations. Methods include log-return transformations, stationarity testing, heteroskedasticity- and autocorrelation-consistent (HAC) level and return regressions, and exploratory vulnerability indicators. \vTwo{As a contextual extension, the article also considers a shorter territorial exposure layer based on disaster-event records for Santander, which is interpreted cautiously as local stress information rather than as a co-equal market-transmission system.} Preliminary results indicate that international cocoa prices are the dominant short-run driver of Colombian cocoa price changes, whereas the European downstream price index adjusts more slowly and less strongly. Weather variables add limited explanatory power to the core transmission models but strengthen the interpretation of smallholder vulnerability by capturing local environmental stress alongside market volatility. \vTwo{The contextual shock layer provides exploratory evidence on when benchmark exposure may coincide with territorial disruption, but those patterns should be interpreted cautiously.}
\end{abstract}
\textbf{Keywords:} cocoa price volatility; price transmission; smallholder vulnerability; territorial exposure; Colombia; European downstream prices; weather context


\section{Introduction}

Cocoa price volatility is not only a market outcome. In vulnerability and food-system resilience research, shocks matter because exposure interacts with sensitivity and adaptive capacity to shape harm \citep{Tendall2015,Zurek2022}. In cocoa-producing regions, producers face price instability together with climate stress and market positions that are often structurally weak. Cocoa studies report that producer concerns include price volatility, weak farmer organizations, dependence on few buyers, and climate-related production risks \citep{Mithofer2017,AkrofiAtitianti2018}. Agroforestry research further shows that the loss of ecological buffering can reduce farmers' ability to adapt to both cocoa price volatility and climate change \citep{Vaast2014}. In that setting, vulnerability is shaped not simply by low prices, but by the interaction between transmitted market shocks and limited capacity to absorb them \citep{Turner2003,Adger2006,TsowouGayi2019}.

That framing leads to a more specific question: how do external shocks enter the livelihood system in the first place? Food-system analysis emphasizes that market, environmental, and institutional pressures interact across scales rather than arriving as isolated disturbances \citep{Ericksen2008,Zurek2022}. Evidence from cocoa-market integration shows that producer exposure intensifies when reforms and market integration strengthen the link between world and domestic cocoa prices, even when farmers do not capture a commensurate share of the gains \citep{TsowouGayi2019}. Exchange-rate volatility can also alter cocoa price dynamics through the financial side of commodity markets, while broader pricing-chain evidence shows that commodity and energy shocks are transmitted unevenly to producer and consumer prices \citep{Jumah2001,JimenezRodriguez2022}. Comparable evidence from coffee, another tropical perennial cash crop cultivated under similar climatic and organizational conditions, reinforces the broader point that adaptive capacity depends on value-chain organization and adaptation support rather than on farm-level decisions alone \citep{Gereffi2005,Morales2022}. The resilience problem is therefore not only climatic or institutional; it is also about the mechanism through which global volatility becomes local producer exposure \citep{Merkle2021}.

A parallel literature examines climate vulnerability and adaptation within cocoa systems. Cocoa vulnerability has been analysed in relation to climatic suitability and limits to adaptation \citep{Schroth2016,Laderach2013}, while physiological and farm-system studies show that cacao is sensitive to water and heat stress and that agroforestry can improve buffering capacity under climate pressure \citep{Lahive2019,Niether2020,Jacobi2013,Vaast2014}. Comparable climate-risk work on coffee should be read as cross-crop rather than cocoa-specific evidence, but it supports the more general proposition that perennial tropical cash crops face spatially differentiated environmental risk and adaptation requirements \citep{Koh2020,Laderach2016}. Across much of this literature, market-oriented and climate-oriented analyses are still operationalized separately: market studies focus on integration and pricing risk, while climate studies focus on suitability, physiology, and adaptation \citep{TsowouGayi2019,Jumah2001,Schroth2016,Lahive2019}. \vTwo{A related challenge appears in territorial risk studies, where local disaster or crisis conditions are often documented descriptively but are less often connected to the supply-chain transmission channels through which international instability reaches producer territories.}

This article therefore studies cocoa price transmission first and reads local vulnerability through that transmission channel. It examines whether movements in international cocoa benchmarks are transmitted to Colombian domestic cocoa prices and European downstream chocolate prices, and whether local exposure conditions should be interpreted mainly as contextual stressors rather than as dominant short-run price drivers. \vTwo{The territorial exposure layer is intentionally modest: it is used to ask whether months of local disruption may help qualify how benchmark-driven exposure is experienced in smallholder-producing territories, not to claim that local event records replace benchmark or macroeconomic drivers.} This design keeps the focus on prices and volatility while allowing a cautious bridge to territorial vulnerability.

The remainder of the manuscript is organized as follows. Section~\ref{sec:litreview} reviews the conceptual and empirical background. Section~\ref{sec:gap} states the research gap and contribution. Section~\ref{sec:data} presents the data architecture. Section~\ref{sec:methods} describes the methodology. Section~\ref{sec:results} reports the results. Sections~\ref{sec:discussion}, \ref{sec:policy}, and \ref{sec:limitations} discuss interpretation, policy implications, and limitations. Section~\ref{sec:conclusion} concludes. \vTwo{Appendix material reports the contextual local-shock diagnostics retained for transparency.}

\section{Literature review and conceptual background}
\label{sec:litreview}

The Introduction established the paper's central claim: resilience in cocoa systems cannot be understood only through climate stress or only through prices, because producer vulnerability is shaped by how external shocks are transmitted and absorbed. That formulation is consistent with vulnerability frameworks in which harm depends on both exposure and the capacity of the affected system to respond \citep{Turner2003,Adger2006}. The literature review sharpens that point by showing that the relevant gap is threefold and internally connected.

The first element of the gap concerns transmission itself. Cocoa studies explain market integration, pass-through, and exchange-rate exposure, but they rarely embed those processes in a resilience framework. Existing evidence shows that deeper integration into world markets can increase farmer exposure to international volatility, while exchange-rate conditions reshape cocoa price risk through distinct commodity-market channels \citep{TsowouGayi2019,Jumah2001}. More broadly, price-transmission research shows that pass-through along pricing chains is often partial, heterogeneous, and method-sensitive \citep{MeyerCramon2004,acosta_ihle_voncramon_2019}. These findings justify modelling world cocoa prices and the COP/USD rate as separate transmission channels rather than treating external volatility as an undifferentiated shock. At the same time, they leave open the question of how far benchmark shocks travel across linked market layers once producer and downstream prices are examined together.

The second element concerns where adjustment is absorbed along the chain. Global value-chain and food-system research suggest that vulnerability is not simply an attribute of producers; it is also shaped by how institutions, market power, and support systems distribute adjustment across actors \citep{Gereffi2005,Merkle2021}. Producer concerns in cocoa commonly include price volatility, weak farmer organizations, dependence on few buyers, and limited alignment between interventions and producer priorities \citep{Mithofer2017}. Comparative work on coffee value chains, used here only as evidence from a similar tropical tree-crop system, reaches a related conclusion by showing that adaptive capacity depends on coordination, support, and innovation across the chain \citep{Morales2022,Verburg2019}. More general pricing-chain evidence also suggests that commodity shocks need not pass through symmetrically across market layers \citep{MeyerCramon2004,JimenezRodriguez2022}. This combined evidence implies that adjustment may be experienced differently across actors, but the retained cocoa studies used in this manuscript do not jointly estimate benchmark, producer-linked, and downstream consumer prices in one aligned monthly framework \citep{TsowouGayi2019,Jumah2001}. % PLACEHOLDER: add direct cocoa evidence on producer-versus-downstream price asymmetry here if a vetted Q1/Q2 source is retained in a later draft.

The third element concerns the status of weather in the analysis. In vulnerability analysis, exposure to hazards is analytically distinct from the sensitivity and resilience of the affected system \citep{Turner2003,Adger2006}. Climate-resilience research documents environmental stress in cocoa systems, but usually without linking that stress to formal price-transmission analysis. Cocoa-specific studies show that vulnerability depends on climatic suitability, physiological sensitivity, and ecological design, with agroforestry improving buffering capacity under heat and moisture stress \citep{Schroth2016,Lahive2019,Niether2020}. Comparable climate-risk work on coffee, interpreted cautiously as cross-crop evidence, supports the more general proposition that adaptation requirements are spatially differentiated across perennial tropical cash-crop systems \citep{Koh2020,Laderach2016}. Prior local cocoa modelling further supports the NASA POWER weather architecture used here for San Vicente de Chucur\'{\i} \citep{talero_sarmiento_etal_2025}. Together, these studies justify treating weather as contextual stress in the present paper, but they do not show that local weather should replace benchmark transmission as the dominant short-run mechanism.

\vTwo{A fourth strand of literature is relevant for the unified framing. Territorial risk and disaster studies often use event registries, hazard counts, or composite pressure indicators to describe local disruption, yet these measures are usually interpreted separately from commodity-price pass-through. That separation is understandable because local event records are episodic, spatially uneven, and often not directly comparable to continuous market series. Even so, they can still be informative when they are used cautiously to describe the conditions under which benchmark exposure is experienced. In this article, that literature supports a contextual reading of local shocks rather than a claim of direct short-run price leadership.} \placeholder{Add one or two vetted citations on territorial disaster indicators or contextual hazard indices if these references will remain in the final journal draft.}

The gap is therefore not the absence of climate research or market research in isolation, but the absence of a design that links them without overstating causality. Across the strands reviewed above, transmission, chain governance, and environmental stress are usually analysed separately rather than in one aligned empirical system \citep{TsowouGayi2019,Schroth2016,Mithofer2017}. This article responds by estimating transmission from international cocoa benchmarks to Colombian producer-linked prices, comparing that adjustment with a downstream European chocolate price index, and then using localized weather indicators to interpret when market-driven exposure is experienced under stronger environmental stress. \vTwo{It also considers whether a shorter territorial shock layer can sharpen that interpretation without displacing the primary focus on benchmark transmission and price volatility.} The methodological contribution lies in keeping these layers analytically distinct: transmission is identified in the core models, weather enters as a contextual extension under explicit sample limits, and \vTwo{local event records are treated as contextual exposure evidence under even tighter overlap and stronger caution.}

\section{Research gap and contribution}
\label{sec:gap}

Existing studies often examine commodity price volatility, pass-through, or climate and disaster vulnerability separately, but fewer studies connect international cocoa price transmission with the local exposure conditions faced by smallholder-producing territories. In practical terms, three linked questions follow from that gap. First, how strongly do international cocoa shocks pass through to producer-linked Colombian prices once exchange-rate and oil-price conditions are included, and how does that adjustment compare with downstream European chocolate prices? Second, is adjustment distributed unevenly along the chain, leaving producer-linked prices more exposed than downstream consumer prices? Third, can local exposure conditions be incorporated without conflating contextual stress with the core market-adjustment mechanism?

The contribution of the article is threefold. First, it places producer-linked Colombian prices, world cocoa benchmarks, and European downstream chocolate prices in one aligned empirical framework, allowing the supply chain to be analysed as a transmission system rather than as disconnected markets \citep{acosta_ihle_voncramon_2019,JimenezRodriguez2022}. Second, it links that transmission question to vulnerability by treating weather as local context that can amplify exposure even when it does not dominate short-run price formation, consistent with frameworks that distinguish exposure from the conditions that shape realized harm \citep{Turner2003,Adger2006}. \vTwo{Third, it introduces a cautious territorial exposure layer based on local event records to assess whether short windows of disruption can be used to contextualize benchmark-driven vulnerability without turning the article into a separate disaster paper.} \placeholder{Clarify whether the unified contribution should be described primarily as empirical, methodological, conceptual, or some combination of the three.}


\section{Data}
\label{sec:data}

The data architecture follows the paper's three research questions. The core system combines Colombian cocoa prices, the world cocoa benchmark, the COP/USD exchange rate, Brent oil prices, and the EU chocolate index. A separate local block contains NASA POWER weather indicators for San Vicente de Chucur\'{\i}, Santander. \vTwo{A shorter contextual layer contains local event information intended to describe territorial disruption in the producing region.} This sequencing is deliberate: prices and macro controls identify transmission, downstream prices identify supply-chain adjustment, weather identifies the local stress setting, and \vTwo{event records are used only to contextualize exposure under a shorter nested window.}
...
\subsection{Contextual Local Shock Data}

\vTwo{The unified manuscript also draws on a shorter contextual dataset of local events for Santander. The working material in the repository indicates 594 event records spanning August 2021 to July 2024, with 91 municipalities and 25 event types represented in the aggregated file used for the crisis-oriented extension. In the present article, this dataset is not treated as a second market system. Instead, it is used to identify whether months of territorial disruption overlap with periods of stronger benchmark-driven exposure.} \placeholder{Insert exact institutional source name, retrieval path, and data governance description for the Santander event registry.} \placeholder{Clarify whether the spatial scope is departmental, municipal, or mixed after aggregation to the monthly analytical layer.}
...
\vTwo{Santander event layer} & \vTwo{Monthly event counts and contextual pressure score} & \vTwo{Count / index} & \placeholder{Source} & \vTwo{2021-08 to 2024-07} \\
\bottomrule
\end{longtable}
\endgroup
...
\vTwo{Nested contextual event window} & \vTwo{36} & \vTwo{2021-08 to 2024-07} & \vTwo{Local event layer for contextual exposure only} \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
...
The rationale for this approach is both methodological and substantive. It avoids imposing artificial cross-block relationships during imputation, such as inferring missing weather observations directly from price variables or reconstructing missing price observations from climatic covariates. The preferred approach is iterative multivariate imputation, which preserves the internal correlation structure within each block \citep{vanbuuren_groothuisoudshoorn_2011}. K-nearest-neighbors (KNN) imputation is retained as a sensitivity benchmark in order to assess whether the completed values are materially dependent on the chosen algorithm \citep{troyanskaya_etal_2001}.
...
This second approach is conceptually attractive because it asks whether climatic conditions enter the same long-run adjustment mechanism as international benchmarks, domestic producer prices, downstream European prices, and macroeconomic controls. However, the climate-augmented sample currently comprises only 51 complete monthly observations, rendering the system empirically fragile for formal reporting. For that reason, the present draft treats the augmented vector error-correction model (VECM) as an exploratory extension and a possible robustness check rather than as the paper's main empirical claim \citep{johansen_1991,hakkio_rush_1991}.

\subsection{Contextual Local Shock Layer}

\vTwo{The local-shock extension is designed as a contextual overlay, not as a co-equal transmission block. Event records are available only for a shorter nested window, they are episodic rather than continuous, and some hazard types are too sparse to support direct time-series use. For that reason, the article treats local events as markers of territorial stress that may qualify how benchmark-driven exposure is experienced in the producing territory.} \vTwo{The practical screening logic is simple: candidate monthly event signals are compared on temporal coverage, number of non-zero months, and interpretability. Under the current repository evidence, hydrometeorological events appear more informative than isolated earthquake counts for identifying months of local disruption. Composite pressure indicators based on multiple event features may still be useful for descriptive context, but they should be read as synthetic territorial markers rather than as market-leading variables.} \placeholder{Clarify whether hydrometeorological counts, a composite pressure index, or both should remain in the unified empirical narrative.} \placeholder{Confirm whether local-event variables enter any regression table in the main text or remain an appendix-level contextual layer only.}
...
\subsection{Core transmission results}
...
\caption{Impulse-response matrix for the core return vector autoregression (VAR). Acronyms identify the return series for world cocoa, Colombian cocoa, and the EU chocolate index.}
\label{fig:irf}
\end{figure}

\subsection{Weather and contextual vulnerability results}
...
\subsection{Contextual local shocks and territorial exposure}

\vTwo{The repository material suggests that local shock indicators are best interpreted as contextual territorial markers rather than as primary price drivers. The shorter event window captures months of local disruption in Santander, but its temporal overlap is much narrower than the core price system and some event categories appear too sparse for stable use in regression-heavy specifications. In that setting, the most defensible role for the event layer is to indicate when benchmark-driven exposure may have coincided with local stress conditions.} \vTwo{This reading is consistent with the broader results. The price system remains dominated by the transmission block, while weather contributes mostly to contextual vulnerability interpretation. The local event layer sharpens that interpretation by identifying a territorial dimension of exposure that does not appear directly in benchmark, exchange-rate, or downstream series.} \placeholder{Confirm whether any local-shock coefficient is strong enough to report in the main text or whether all event-based results should remain descriptive and appendix-based.}

\subsection{Integrated reading of the findings}

Three results stand out. First, the effect of world cocoa returns on Colombian cocoa returns is positive, economically large, and statistically strong in the aligned sample. Second, the EU downstream series moves with the system in levels, but the short-run response is much weaker and the downstream return model has poor fit. Third, weather variables do not dominate the market system, but they improve the vulnerability interpretation by locating periods in which market-driven exposure is experienced under stronger local stress. \vTwo{The shorter territorial shock layer points in the same direction, but it should be interpreted as exploratory contextual evidence rather than as proof of a separate local price mechanism.}
...
\vTwo{The contextual event layer points in the same direction, but under stricter limits. Local event records can help identify months of territorial disruption, yet they do not justify reframing the article around disaster causality. Their value lies in qualifying benchmark exposure, not in displacing the price-transmission mechanism that remains central to the results.}

\section{Policy implications}
\label{sec:policy}

The policy implication is not that producer resilience can be strengthened through climate adaptation measures alone. The current evidence points instead to a broader diagnostic frame in which benchmark pass-through, the macroeconomic environment of transmission, differences in adjustment across market layers, and locally weather-sensitive support systems all matter together \citep{Gereffi2005,Merkle2021}. Policies that target only agronomic adaptation may miss an important part of the exposure mechanism if international price instability continues to reach producer territories quickly.

\vTwo{A cautious territorial implication follows from the local-shock layer. If local event records are retained as contextual evidence, they support stronger coordination between market monitoring and territorial risk management. Early-warning systems, transport contingency planning, local storage, cooperative coordination, and emergency response capacity may matter most in periods when benchmark volatility and territorial disruption overlap.} \placeholder{Clarify which policy domain should be emphasized most strongly in the final target journal draft: producer organizations, territorial governance, market regulation, adaptation policy, or supply-chain coordination.}
...
\vTwo{The contextual event layer adds further limitations. The shorter overlap, likely aggregation across municipalities, and uneven event density make it unsuitable for strong causal claims. Event records can still inform interpretation, but only if they are used as contextual exposure evidence and if their exact source, temporal coverage, and spatial aggregation are documented more clearly than they are in the current repository materials.}

\section{Conclusions}
\label{sec:conclusion}

This article presents a transparent empirical framework for studying cocoa price volatility and smallholder vulnerability across the supply chain. Its main contribution is to address three linked questions within one design: how international cocoa shocks are transmitted to Colombian producer-linked prices once exchange-rate and oil conditions are considered, whether adjustment is distributed unevenly between producer-linked and downstream prices, and whether local exposure conditions should be interpreted as contextual stress rather than as the dominant short-run price driver.

The evidence points to three main conclusions. First, transmission from international cocoa markets to Colombian domestic prices is strong and statistically robust, indicating that producer exposure is closely tied to benchmark movements within a broader macroeconomic environment. Second, the European downstream chocolate index shows weaker and less stable short-run adjustment, which is consistent with, but does not by itself prove, uneven distribution of adjustment across actors in the supply chain. Third, the weather variables do not overturn the core transmission results, and their main value in the current draft is to identify the local stress context in which market-driven exposure is experienced.

Taken together, these results suggest that vulnerability in the cocoa chain is shaped by transmitted market instability, the macroeconomic setting of transmission, differences in adjustment across market layers, and local stress conditions. The article therefore contributes to resilience research by showing that market transmission itself is part of the exposure mechanism that vulnerability analysis must explain \citep{Turner2003,Adger2006}. \vTwo{The shorter territorial shock layer adds a cautious extension: local event records may help identify when benchmark exposure coincides with territorial disruption, but the evidence remains exploratory and should not be overstated.} For policy, the implication is that producer resilience cannot be strengthened through climate adaptation measures alone; it also requires attention to benchmark pass-through and to the institutional conditions that determine how shocks are absorbed or passed along the chain \citep{Gereffi2005,Merkle2021}. \placeholder{Confirm whether conclusions should refer to transmission, association, or only descriptive co-movement for the downstream and contextual local-shock results.}
...
\clearpage
\appendix
\section{Appendix: Contextual local-shock diagnostics}

\placeholder{Check whether appendix figures use a fully harmonized palette with the core manuscript figures before journal submission.}

\vTwo{The figures in this appendix are retained only to document the shorter contextual local-shock layer discussed in the main text. They should be read as descriptive diagnostics for territorial exposure rather than as evidence of a separate market-transmission system.}
...
\caption{\vTwo{Monthly local-event totals and hazard-domain composition in the Santander contextual layer. The displays summarize territorial disruption in the shorter event window and are included to document local exposure conditions rather than to imply direct price leadership.}}
\label{fig:appendix_event_dynamics}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{figures/figure_contextual_overlay_alignment.png}
\caption{\vTwo{Illustrative alignment between core return series and contextual local-shock markers in the shorter nested window. This diagnostic is retained to show timing overlap, not to claim that local events form a co-equal benchmark system.}}
\label{fig:appendix_contextual_alignment}
\end{figure}

\begin{table}[H]
\centering
\caption{\vTwo{Summary of contextual local-shock coverage used for appendix-level diagnostics}}
\label{tab:appendix_local_shock_data}
\small
\begin{tabular}{p{6.0cm}p{5.0cm}}
\toprule
Item & Value \\
\midrule
\vTwo{Contextual event records} & \vTwo{594} \\
\vTwo{Observation window} & \vTwo{2021-08 to 2024-07} \\
\vTwo{Distinct municipalities} & \vTwo{91} \\
\vTwo{Distinct event types} & \vTwo{25} \\
\vTwo{Observed earthquake-related events} & \vTwo{4} \\
\vTwo{Intended role in unified manuscript} & \vTwo{Contextual territorial exposure layer} \\
\bottomrule
\end{tabular}
\normalsize
\end{table}

\placeholder{Clarify whether hydrometeorological counts, composite pressure indices, or both should be retained in the final appendix set.}
