...
This second approach is conceptually attractive because it asks whether climatic conditions enter the same long-run adjustment mechanism as international benchmarks, domestic producer prices, downstream European prices, and macroeconomic controls. However, the climate-augmented sample currently comprises only 51 complete monthly observations, rendering the system empirically fragile for formal reporting. For that reason, the present draft treats the augmented VECM as an exploratory extension and a possible robustness path rather than as the main empirical claim of the paper \citep{johansen_1991,hakkio_rush_1991}.

\subsection{Exploratory Vulnerability Indicators}

The weather chapter also constructs three exploratory indicators that are used for interpretation rather than as validated welfare scales. The first step is to summarize contemporaneous local climatic stress from the selected weather variables: precipitation, surface solar radiation, and maximum temperature. Let $w_{j,t}$ denote the value of weather variable $j$ in month $t$, and let $z(w_{j,t})$ denote its within-sample standardized value. The weather-stress index is then defined as:
\begin{equation}
\text{WeatherStressIndex}_t = \frac{1}{J} \sum_{j=1}^{J} \left| z(w_{j,t}) \right|,
\label{eq:weather_stress_index}
\end{equation}
where $J=3$ in the current draft. By construction, this index is larger when the month exhibits bigger absolute deviations from the aligned-sample mean across the selected weather dimensions, regardless of sign. The plotted weather-stress series is the standardized version of that composite:
\begin{equation}
\text{WeatherStressZ}_t = z(\text{WeatherStressIndex}_t).
\label{eq:weather_stress_z}
\end{equation}

Positive values of $\text{WeatherStressZ}_t$ therefore indicate above-average combined weather anomaly within the aligned sample, while negative values indicate relatively mild weather conditions in comparative terms.

The farmer exposure index combines three standardized channels: domestic cocoa volatility, the magnitude of the fitted market-transmission shock from the core domestic return model, and weather stress. Let $\sigma^{COL}_t$ denote rolling domestic return volatility and let $\widehat{m}_t = \left| \widehat{\Delta \ln(P^{COL}_{t})} \right|$ denote the absolute fitted value from the core short-run transmission equation. The resulting indicator is:
\begin{equation}
\text{FarmerExposureIndex}_t = z(\sigma^{COL}_t) + z(\widehat{m}_t) + \text{WeatherStressZ}_t.
\label{eq:farmer_exposure}
\end{equation}

This index should be interpreted as an exposure score: it rises when domestic volatility is elevated, when the fitted market-transmission shock is large in magnitude, and when local weather conditions are unusually stressful. Finally, the livelihood-risk score adds a dependence-like external pressure term based on world cocoa volatility:
\begin{equation}
\text{LivelihoodRiskScore}_t = \text{FarmerExposureIndex}_t + z(\sigma^{WLD}_t),
\label{eq:livelihood_risk}
\end{equation}
where $\sigma^{WLD}_t$ denotes rolling world-cocoa return volatility. In the present draft, these indicators are best read as transparent composite heuristics for vulnerability-oriented interpretation, not as structural welfare measures or causal estimates of household outcomes.
